...
| 5 | フロントエンド SPA | Vite SPA on S3+CloudFront、名前入力でセッション開始（認証なし） | C-7 = A |
| 6 | 外部メッセージング送信 | const ホワイトリスト + DRY_RUN | C-8 = B + C-8a = C **最重要安全境界** |
...
| ID | サービス | 主トリガ |
|---|---|---|
| S1 | オンボーディング（名前入力でセッション開始） | フロント初回起動 |
...
| C-7 (SPA) と WebSocket 連携 | ✅ WebSocket クライアント実装で対応可能（認証は MVP 撤廃、user_id は body / header で受け渡し） |
...
- **Unit 4**: 音声 UI（Polly + WebSocket 配信、フロント音声プレイヤー）
- **Unit 5** (要検討): 外部メッセージング送信（独立 Unit として切るか、Unit 2 に統合するか）
- **Unit 6** (要検討): フロントエンド SPA（独立か、各 Unit に分散するか）
- ~~Unit 認証基盤~~（**2026-05-02 撤廃**、マルチユーザー対応時に `TODO_construction.md` から復帰）
