...
### Follow-up C-AgentCore: AgentCore 採用範囲（2026-05-03 追加）

**user 指示**: "オブサバビリティーでagentcore使わないとデバッグ詰むのかと思っている" → "AgentCore RuntimeはGAしてますよ！！ファクトチェック"

**確定事項**: **AgentCore Runtime + Observability のみ採用**（他は MVP 不採用）。

| AgentCore 要素 | 採用 |
|---|---|
| Runtime (GA) | ✅ |
| Observability | ✅ |
| Gateway | ❌（1 Agent + tool 4-6 個ではオーバーヘッド） |
| Memory | ❌（DynamoDB で代替） |
| Identity | ❌（認証撤廃済） |
| Multi-agent Collaboration | ❌（1 Agent 構成） |
| Code Interpreter / Browser | ❌（MVP 不要） |

**採用根拠**:
- AgentCore Observability の Replay / Compare 機能は AI agent デバッグの差別化要素、代替手段（Bedrock Model Invocation Logging + X-Ray）では取れない
- Runtime / Observability ともに **GA 済 (2026-05 時点)**、preview リスクなし
- Multi-agent / Memory / Gateway は MVP 規模では過剰、将来 park 項目で復帰検討（`TODO_construction.md` 「AgentCore フル機能採用への移行」）

**詳細**: `requirements.md` Appendix B.10、`unit-of-work.md` §2 Unit-B 参照。
